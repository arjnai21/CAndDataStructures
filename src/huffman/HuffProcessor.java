...
public class HuffProcessor implements Processor {
    ...
    private HuffNode makeTreeFromCounts(int[] array) {
        ...
        while (tree.size() > 1){
            HuffNode left = tree.remove();
            HuffNode right = tree.remove();

            HuffNode sum = new HuffNode(-1, left.weight()+ right.weight(), left, right);
            tree.add(sum);
        }
        return tree.remove();
    }
    ...
    private String[] makeCodingsFromTree(HuffNode root) {
        ...
        makeCodingsFromTree(root, "", arr);
        return arr;
    }

    private void makeCodingsFromTree(HuffNode node, String coding, String[] arr) {
        if (node.value() != -1){ // leaf node
            arr[node.value()] = coding;
            return;
        }
        makeCodingsFromTree(node.left(), coding + "0", arr);
        makeCodingsFromTree(node.right(), coding + "1", arr);
    ...
    private void writeCompressedBits(BitInputStream in, String[] codings, BitOutputStream out)
    {
        int bits;
        while((bits = in.readBits(BITS_PER_WORD)) != -1){
            String strCode = codings[bits];
            int code = Integer.parseInt(strCode, 2);
            out.writeBits(strCode.length(), code);
        }
        String strCode = codings[256];
        int code = Integer.parseInt(strCode, 2);
        out.writeBits(strCode.length(), code);
    ...
    HuffNode readHeader (BitInputStream in) {
        ...
        if (bit == 0){
            HuffNode next = readHeader(in);
            HuffNode next2 = readHeader(in);
            return new HuffNode(-1, -1, next, next2);

        }

        int letter = in.readBits(9);
        return new HuffNode(letter, -1);
